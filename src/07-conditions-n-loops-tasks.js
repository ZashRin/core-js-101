...
function findFirstSingleChar(str) {
  ...
  const chars = Object.keys(o);
  const count = Object.values(o);
  return chars[count.indexOf(1)];
}
...
function getMatrixProduct(m1, m2) {
  const n = m1.length; const m = m1[0].length; const
    p = m2[0].length;
  const c = Array(n).fill().map(() => Array(p).fill(0)); let
    sum;
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < p; j += 1) {
      sum = 0;
      for (let k = 0; k < m; k += 1) sum += m1[i][k] * m2[k][j];
      c[i][j] = sum;
    }
  }
  return c;
